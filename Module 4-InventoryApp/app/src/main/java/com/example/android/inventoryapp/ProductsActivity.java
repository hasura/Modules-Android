...
public class ProductsActivity extends AppCompatActivity {
    ...
    public class SectionsPagerAdapter extends FragmentPagerAdapter {
                ...
                .enqueue(new Callback<List<CategoryResponse>, HasuraException>() {
                    ...
                    public void onSuccess(List<CategoryResponse> categoryResponses) {
                        ...
                        for(i = 0;i<categories.size();i++){
                            ...
                            client.useDataService()
                                    .setRequestBody(new SelectQuery(categories.get(i).getCategory()))
                                    ...
                                    .enqueue(new Callback<List<ItemModel>, HasuraException>() {
                                        ...
                                        public void onSuccess(List<ItemModel> itemModels) {
                                            category.setItemList(itemModels);
        ...
        linearLayoutManager = new LinearLayoutManager(ProductsActivity.this);
        adapter = new CategoryListAdapter(new CategoryListAdapter.Interactor() {
            @Override
            public void onButtonClicked(List<ItemModel> itemList) {
                Global.currentItemList = new ArrayList<>();
                Global.currentItemList.addAll(itemList);

                Intent i = new Intent(ProductsActivity.this,ViewAllItems.class);
                startActivity(i);
            }
        });
...
/*
    my idea is to display something like flipkart

    i will have a recyclerview with a linear layout manager

    in each of these linear layout items, i have to add a grid layout(optional)
    or hardcode 4 images...say top 4.

    when i display i will first segregate acc to categories.

    while segregating i can either pull all at once and then sort
    or pull acc to categories.(2nd seems easier to implement
    only i will have to fire more queries.

    CLick on any product will take to the individual product screen.
